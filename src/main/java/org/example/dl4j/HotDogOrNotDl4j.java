...
public class HotDogOrNotDl4j {
    ...
    private static int channels = 3; // RGB
    ...
    private static int epochs = 2; // original 100 - most effective, but lengthiest
    ...
    public static void training() throws IOException {
        ...
        numLabels = Objects.requireNonNull(
                test.getRootDir().listFiles(File::isDirectory)).length; //This only works if your root is clean: only label subdirs.

        /*
         *  Create extra synthetic training data by flipping, rotating images on our data set.
         */
        ...
        try (ImageRecordReader recordReader = new ImageRecordReader(height, width, channels, labelMaker)) {
            ...
            // Test data should be resized and normalized, not augmented
        ...
        File basePath = new File("src/main/resources/models/");
        ModelSerializer.writeModel(network, basePath + "hotdog.bin", true);
